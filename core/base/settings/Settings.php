...
class Settings
{
    ...
    // Те таблицы, содержащие данные для фильтрации, категории для них добавляется
    // тип для записей 'корневая'. Например в случае когда записи таблицы ссылаются на
    // первичные ключи записей своей же таблицы. Пример в таблице categories в
    private $rootItems = [
        'name' => 'корневая',
        // таблицы имеющие корневую категорию
        'tables' => ['filters', 'categories'],
    ];

    // данные для формирования html-блоков в контентной части () страницы админ.части сайта
    ...
    // настройки для связи записей таблиц БД по типу многие ко многим
    // данные для формирования html-блока контентной части страницы админ.части сайта
