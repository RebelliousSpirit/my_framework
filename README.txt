1) Принцип работы сайта:
   ...
   4) TRANCATE - команда sql для удаления записей в БД и при этом полностью обновляет первичный ключ
   ...
   9) В этом проетке мы создаем мини ORM. Расположенна он а в директории core/base/models
        ...
        11) Инструкция JOIN языка SQL является одно из самых быстрых в выполнении. UNION и вложеные запросы работают
        медленней

Структура БД:
    - articles
    - categories
    - filters - в фильтрах поля сслаются на другие поля внутри таблицы
    - goods
    - parsing_data - хранит данные парсинга сайта
    - old_alias - хранит старые псевдонимы ссылок. Эти данные нужны для SEO
     ИНФОРМАЦИЯ для проектирования таблиц БД
     - для того чтобы связать две таблицы по ключам по типу один к многим:
        Например создаем таблицу filters с полями: id(первичный ключ), name(varchar(255)), visible(int(11)), parent_id(
        внешний ключ int(11)). Создаем таблицу filters_categories с полями:  id(первичный ключ), name(varchar(255)).
        Для того чтобы связать поля таблицы filters по ее parent_id(внешний ключ) с полями filters_categories по ее id
        (первичный ключ) НУЖНО модифицировать таблицу filters: добавить внешнии ключ. ВАЖНО чтобы в таблице filters
        в полях parent_id не было null и они не ссылались на поля своей же таблицы.
...
Данные о внешних ключах хранятся в отдельной таблице БД с именем information_schema.
...
Нам нужно получить данные полей, которые сслыаются по внешнему ключу на первичный(е) ключи полей этой же таблицы
для этого в запросах нужно использовать псевдоним текущей таблицы(alias) при формировании запроса, иначе sql-сервер вы
даст ошибку.
33) Разрабатываем метод для формирования данных "связи многие ко многим". СОГЛАШЕНИЕ: название таблицы содержащей данные
связанных таблиц должно иметь имя осставленное из 2 имен этих таблиц. Например: 1 таблица 'products', 2 таблица 'categories',
значит таблица для хранения их связей должна называться 'products_categories'
